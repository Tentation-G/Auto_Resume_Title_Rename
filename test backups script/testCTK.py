...
class App(ctk.CTk):
    ...
    def on_validate(self):
        ...
        # Nettoyer le texte pour en faire un suffixe de nom de fichier valide
        safe_suffix = re.sub(r'[^A-Za-z0-9_-]', '', new_txt.replace(' ', '_'))
        if not safe_suffix:
            print("Erreur : le texte saisi n'est pas valide pour un nom de fichier.")
            return
        pdf_out = f"{FILE_OUT_PREFIX}{safe_suffix}.pdf"

        try:
            modif_resume_title(FILE_IN, pdf_out, SEARCH_TEXT, new_txt)
